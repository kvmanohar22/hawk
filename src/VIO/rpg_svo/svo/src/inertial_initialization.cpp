...
InertialInitialization::InertialInitialization(
  ...
  Vector3d gravity) :
    ...
    window_len_msgs_(static_cast<int>(0.4/Config::dt())),
...
bool InertialInitialization::initializeFast()
{
  if(imu_msgs_.empty())
    return false;
  ...
  // collect windowed messages
  list<sensor_msgs::Imu::ConstPtr> imu_msgs_new;
  const double first_msg_time = imu_msgs_.front()->header.stamp.toSec();
  for(auto itr=imu_msgs_.begin(); itr!=imu_msgs_.end(); ++itr)
  {
    const double prev_msg_time = (*itr)->header.stamp.toSec(); 
    if(prev_msg_time > first_msg_time && prev_msg_time < first_msg_time+window_len_)
      imu_msgs_new.push_back((*itr));
  }
  if(imu_msgs_new.size() < window_len_msgs_)
  {
    SVO_WARN_STREAM_THROTTLE(0.4, "Empty window messages: size = " << imu_msgs_.size() << "\t current = " << imu_msgs_new.size() << "\t target = " << window_len_msgs_);
    return false;
  }
  SVO_INFO_STREAM("n_messages = " << imu_msgs_new.size());

  // computes stats
  Vector3d acc_avg; acc_avg.setZero();
  for(auto itr=imu_msgs_new.begin(); itr!=imu_msgs_new.end();++itr)
    acc_avg += ros2eigen((*itr)->linear_acceleration);
  acc_avg /= static_cast<int>(imu_msgs_new.size());
  double new_var=0;
  for(auto itr=imu_msgs_new.begin(); itr!=imu_msgs_new.end();++itr)
  {
    const Vector3d acc = ros2eigen((*itr)->linear_acceleration);
    new_var += (acc-acc_avg).dot(acc-acc_avg);
  }
  new_var = std::sqrt(new_var/(static_cast<int>(imu_msgs_new.size()-1)));
  if(new_var < threshold_)
  {
  ...
  Vector3d omg_avg = omg_sum / imu_msgs_new.size();
  ...
  bias_g_ = omg_avg;
  bias_a_ = acc_avg + R_init_.transpose() * gravity_;
  t0_ = imu_msgs_new.back()->header.stamp.toSec();
  SVO_INFO_STREAM("Inertial initialization succesfull!");
  SVO_INFO_STREAM("Roll Pitch Yaw = " << vk::dcm2rpy(R_init_).transpose()*180/PI);
  return true;
}
