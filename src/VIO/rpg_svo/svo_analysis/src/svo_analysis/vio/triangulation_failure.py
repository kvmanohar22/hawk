...
def analyze(path_1kf, path_3kf):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111, xlabel='KeyFrame ID', ylabel='#points')

    plot(ax, data_1kf, ['r.', 'g.', 'b.', 'k.'], 'thresh=1 ', 2)
    plot(ax, data_3kf, ['r-', 'g-', 'b-', 'k-'], 'thresh=1e-10 ', 2)
...
def plot(ax, data, c, label, w):
  X = data[:, 0]
  ax.plot(X, data[:, 3], c[0], label=label+'obs=2', linewidth=w)
  ax.plot(X, data[:, 4], c[1], label=label+'obs=3', linewidth=w)
  ax.plot(X, data[:, 5], c[2], label=label+'obs=4', linewidth=w)
  ax.plot(X, data[:, 6], c[3], label=label+'obs=4+', linewidth=w)
