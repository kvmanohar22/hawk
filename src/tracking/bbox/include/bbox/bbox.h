...
public:
  Bbox(string modelPath, string configPath, string framework, int backend = 0, int target = 0)
  {
  ...
  void setParams(int _inpW, int _inpH, const vector<string>& _classes, float _confThreshold, float _nmsThreshold,
                 Scalar _mean, bool _swapRB, string _kWinName = "BBOX Output")
  {
    inpWidth = _inpW;
    inpHeight = _inpH;
    classes = _classes;
