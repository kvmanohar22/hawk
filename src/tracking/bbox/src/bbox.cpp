...
tuple<std::vector<int>, std::vector<float>, std::vector<Rect>> Bbox::predict(Mat frame, bool draw)
{
  ...
  Size inpSize(this->inpWidth > 0 ? this->inpWidth : frame.cols, this->inpHeight > 0 ? this->inpHeight : frame.rows);
  blobFromImage(frame, blob, scale, inpSize, this->mean, this->swapRB, false);
  this->net.setInput(blob);

  if (this->net.getLayer(0)->outputNameToIndex("im_info") != -1)
...
void Bbox::postprocess(Mat& frame, const std::vector<Mat>& outs, Net& net, std::vector<int>& classIds,
                       std::vector<float>& confidences, std::vector<Rect>& boxes, std::vector<int>& indices, bool draw)
{
...
void Bbox::drawPred(int classId, float conf, int left, int top, int right, int bottom, Mat& frame)
{
  ...
  rectangle(frame, Point(left, top - labelSize.height), Point(left + labelSize.width, top + baseLine), Scalar::all(255),
            FILLED);
