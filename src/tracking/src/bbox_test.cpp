...
int main(int argc, char** argv) try
{
  ...
  image_transport::Subscriber sub =
      it.subscribe(img_topic, 1, boost::bind(imgCallback, _1, box), ros::VoidPtr(), image_transport::TransportHints());

  // ros::Subscriber sub = nh.subscribe<sensor_msgs::Image>(img_topic, 1, boost::bind(imgCallback, _1,
  // boost::ref(box)));
  ros::spin();
